...
public class MainMenu {
	...
	public static void loginView() {
		...
		Scanner sc = new Scanner(System.in);
		System.out.println(
				"Login View: \nPlease enter your username now, or enter stop to return to the main menu!");
		String input = "";
		sc = new Scanner(System.in);
		input = sc.nextLine();
		...
		if (input.length() < 3) {
			...
			if (Driver.admin.getName().equals(input)) {
				...
				if (Driver.admin.getPassword().equals(input)) {
					System.out.println("\n\n\n\nYou logged in as the admin. Wow! You are an amazing person! =) ");
	...
	public static void registerView() {
		...
		Driver.catReader();
		System.out.println(
				"\nRegister View! Please enter your desired username now, or enter stop to return to the main menu!");
		String input = "";
		sc = new Scanner(System.in);
		input = sc.nextLine();
		if (input.equals("stop"))
			return;
		if (input.length() < 3) {
			System.out.println("\n\n\n\n\n\n\n\nPlease use more than 2 characters for your username.");
			return;
		} else {
			ArrayList<User> users = Driver.userList;
			if(input.equals(Driver.admin.getName())) {
				System.out.println("\n\n\n\n\n\n\n\n\n\n\nYou better not take our admins name, boi!");
				return;
			}
			for (User user : users) {
				if (user.getName().equals(input)) {
					System.out.println("\n\n\n\n\n\n" + input + "is already taken. Maybe try loggin in instead!");
					return;
				}
			}
		}
		String username = input;
		System.out.println("\n\n\n\n\n\n" + username + " is available! Please now choose a password, or enter stop to return to the main menu!");
		sc = new Scanner(System.in);
		input = sc.nextLine();
		if (input.length() < 3) {
			System.out.println("\n\n\n\n\n\n" + "Please use more than 2 characters for your password.");
			return;
		}if (input.equals("\n\n\n\n\n\n" + "stop"))
			return;
		String password = input;
		System.out.println("\n\n\n\n\n\n" + "Your account " + username + " was created. Use " + password + " to login.");
