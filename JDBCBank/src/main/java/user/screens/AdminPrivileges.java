...
public class AdminPrivileges {
	...
	static void createUser() {
		...
		System.out.println("Please enter 1 or 2 for the profile type: ");
	...
	static void updateUser() {
		...
		//Lists all users to choose from
		listUsers();

		//Stores selected userID
		System.out.println("Please type in the User_ID of the user you wish to update. ");
		int userSelect = UserInput.isInt();  //<--Stores input as an int

		CustomerDaoImpl updateCustomer = new CustomerDaoImpl();
		...
		try {
			updateCustomer.updateCustomerName(name, userSelect);  //<--Updates the user's info
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	static void deleteUser(int userID) {
		...
		UserDaoImpl removeUser = new  UserDaoImpl();

		//Stores selected userID
		System.out.println("Please type in the User_ID of the user you wish to delete. ");
		int userSelect = UserInput.isInt();  //<--Stores input as an int

		try {
			removeUser.deleteUser(userSelect);  //<--Deletes the user
		} catch (SQLException e) {
			e.printStackTrace();
		}
